...
export function UserActivityTimeline({ user }: UserActivityTimelineProps) {
  ...
  const events = React.useMemo(() => {
    ...
    list.push({
      ...
      description: "User profile generated and verified in the database.",
      date: user.createdAt,
      type: "system",
    });
    ...
    customerOrders.forEach((order) => {
      if (order.status === "COMPLETED") {
        ...
        list.push({
          ...
          date: new Date(
            new Date(order.createdAt).getTime() + 2000,
          ).toISOString(), // offset slightly for timeline flow
          type: "ownership",
        });
    ...
    <Card>
      ...
      <CardContent className="p-6">
        <div className="relative border-l border-card-border pl-6 space-y-6">
          {events.map((ev, index) => {
            let iconColor =
              "text-neutral-500 bg-neutral-900 border-card-border";
            ...
            return (
              <div key={index} className="relative">
                ...
                <div
                  className={`absolute -left-[37px] top-0.5 flex h-5 w-5 items-center justify-center rounded-full border ${iconColor}`}
                >
                  {icon}
                </div>
