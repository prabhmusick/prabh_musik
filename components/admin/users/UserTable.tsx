...
export function UserTable({ users }: UserTableProps) {
  ...
  const columns: ColumnDef<User>[] = [
    ...
    },
    {
      ...
      header: "Email",
      cell: ({ row }) => (
        <span className="text-neutral-300">{row.original.email}</span>
      ),
    },
    {
      accessorKey: "mobile",
    ...
    },
    {
      ...
      header: "Beats Owned",
      cell: ({ row }) => {
        const count = ownerships.filter(
          (o) => o.customerId === row.original.id,
        ).length;
        return (
          <span className="font-mono text-center block w-fit mx-auto sm:mx-0">
            {count}
          </span>
        );
      },
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const completedSpend = orders
          .filter(
            (o) => o.customerId === row.original.id && o.status === "COMPLETED",
          )
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        if (completedOrders.length === 0)
          return (
            <span className="text-neutral-500 font-mono text-xs">N/A</span>
          );
